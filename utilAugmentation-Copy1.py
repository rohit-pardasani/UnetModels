...
def zoomImage(img,scale_percent,H,W):
    ...
    #print('scale factor:',scale_percent)
    new_width = int(x * scale_percent / 100)
    new_height = int(y * scale_percent / 100)
    dim = (new_width, new_height)
    #print("Zoomed Dimensions :",dim)
    cx=int(new_width/2)
    cy=int(new_height/2)
    #print("New Center:",cx,cy)
    # resize image
    ...
    if(scale_percent>100):
        ...
        #print("lx :",lx)
        #print("ly:",ly)
        dst_img=resized[cy-ly:cy+ry,cx-lx:cx+rx]
        #print('dst image ',dst_img.shape)
        #plt.imshow(dst_img)
        #plt.show()
    else:
        dh=y-resized.shape[0]
        dw=x-resized.shape[1]
        ...
        dst_img = cv2.copyMakeBorder(resized, ldh, rdh, ldw, rdw, borderType = cv2.BORDER_CONSTANT, value=0)
        #crop_mask=resized[]
        #display(dst_img.shape)
        #plt.imshow(dst_img)
        #plt.show()

    x_img = cv2.resize(dst_img, (H, W), interpolation = cv2.INTER_CUBIC)

    x_img = np.float32(x_img)/255
    #plt.imshow(x_img)
    #plt.show()
    x_img = np.reshape(x_img, (H,W,1))
    
    return x_img



def CustomImgGen(indlst,zoomlst,df,H,W,shuffle=False,BATCH_SIZE=16):
    random.seed(3)
    L = len(indlst)
    Z = len(zoomlst)
    while True:
        if(shuffle):
            random.shuffle(indlst)
            random.shuffle(zoomlst)
        ii = 0 # Current image index
        zi = 0 # Current zoom index
        left = Z*L
        while left>0:
            BL = min(BATCH_SIZE,left)
            X_BATCH = np.zeros((BL,H,W,1),dtype=np.float32)
            ...
            for bi in range(BL):
                ...
                # Read image img at imgIdx dataframe df
                # Read mask msk at imgIdx dataframe df
                img=cv2.imread(df.loc[imgIdx,'slice'],0)
                msk=cv2.imread(df.loc[imgIdx,'mask'],0)
                
                zoomLevel = zoomlst[zi]
                #print((imgIdx,zoomLevel))
                
                X_BATCH[bi,:,:,:] = zoomImage(img,zoomLevel,H,W)
                Y_BATCH[bi,:,:,:] = zoomImage(msk,zoomLevel,H,W)
                zi+=1
                if(zi==Z):
                    zi = 0
                    ii+=1
            ...
            left = left - BL
            yield (X_BATCH,Y_BATCH)
            #yield 'batch_generated of len: '+str(BL)

def trainGen(modelName,csvFile,saveFolder,
                    indlist_train,indlist_val,indlist_test,indlist_pred,N_TRAIN_Z,N_VALIDATE_Z,N_TEST,N_PRED_Z,zoomlst,H,W,
                  model,reportCsv,min_lr=0.00001,epochs=200,LR_patience=20,LR_factor=0.1,stop_patience=100,
                    batch_size=2,retrainFlag=False):
    
    
    t_steps = math.ceil(N_TRAIN_Z/batch_size)
    v_steps = math.ceil(N_VALIDATE_Z/batch_size)
    ...
    train_generator=CustomImgGen(indlist_train,zoomlst,df,H,W,shuffle=True,BATCH_SIZE=batch_size)
    val_generator=CustomImgGen(indlist_val,zoomlst,df,H,W,shuffle=True,BATCH_SIZE=batch_size)
